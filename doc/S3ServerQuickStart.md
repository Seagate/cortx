...
3. `$ git clone "ssh://g${GID}@gerrit.mero.colo.seagate.com:29418/s3server"` ( It has been assumed that `git` is preinstalled. if not then follow git installation specific steps provided [here](MeroQuickStart.md). Recommended git version is 2.x.x . Check your git version using `$ git --version` command.)
