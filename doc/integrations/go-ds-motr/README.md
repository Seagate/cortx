...
Several [implementations](https://github.com/ipfs/kubo/tree/master/plugin/plugins) of this data store interface have been developed inluding those using the [LevelDB](https://github.com/google/leveldb) and [BadgerDB](https://github.com/dgraph-io/badger) key-value stores. 
...
```cmd
[root@cortx-ova-rgw go-ds-motr]# time ../kubo/cmd/ipfs/ipfs add "01 Track01.flac"                                                                                                           
...
```cmd
[root@venus go-ds-motr]# time ../kubo/cmd/ipfs/ipfs add "01 Track01.flac"                                                                                                                   
added QmXUdQD5gHs483TCYFTEgFsve4J1sgfM4FGs9XLZzE3obv 01 Track01.flac                                                                                                                           
...
# Requirements
1. Access to a Motr cluster and the `motr` and `motr-devel` packages installed. See the CORTX Motr [repo](https://github.com/Seagate/cortx-motr).
2. Go 1.17. See the official Go [site](https://go.dev/doc/install) for download and install instructions.

# Installation
1. Clone the kubo (formerly go-ipfs) repository: `git clone https://github.com/ipfs/kubo.git`
2. Checkout v0.13.0 of the code: `cd kubo &&  git checkout tags/v0.13.0`
3. Add the following line to the `plugin/loader/preload_list` file: `motrds github.com/allisterb/go-ds-motr/plugin *`. This tells the build system to build the go-ds-motr plugin together with the other plugins specified when building kubo.
4. Make sure the gcc compiler and other essential C++ build tools are installed e.g. on CentOS/Rocky Linux you can say `yum group install "Development Tools"`.
5. Ensure you have the Motr runtime and development libraries and headers installed. You can build Motr yourself by following the instructions on the [Motr repo](https://github.com/seagate/cortx-motr). If you want to test the version of Motr on a CORTX OVA VM release, checkout the version or tag that corresponds to your VM release version e.g. for [CORTX OVA 2.0.0-790](https://github.com/Seagate/cortx/releases/tag/2.0.0-790) you should checkout and build the [2.0.0-790](https://github.com/seagate/cortx-motr/tree/2.0.0-790) version of Motr.
6. Get the Go go-ds-motr module code from GitHub: `go get github.com/allisterb/go-ds-motr/plugin@HEAD`.

7. Build kubo: `make build`. This will create the ipfs executable at `cmd/ipfs/ipfs` in your `kubo` directory.
8. Initialize your IPFS repository: `cmd/ipfs/ipfs init`. This will create an `.ipfs` directory in your `$HOME` directory with your IPFS server configuration.
8. Edit the `config` JSON configuration file In your `$HOME/.ipfs` directory. Change the `Spec` key to the following structure:
    ```json
    "Spec": {
              "child": {
...
10. To test these values before starting the server, clone `https://github.com/allisterb/go-ds-motr` and run the CLI commands to test access to the store e.g:
...
```
11. If you need to create the Motr index you can do that from the CLI too:
```cmd
...
```                                                                                                                                                                                          
12. Set the [UDP receive buffer size](https://github.com/lucas-clemente/quic-go/wiki/UDP-Receive-Buffer-Size) to 2500000 to avoid [this warning message](https://discuss.ipfs.io/t/docker-failed-to-sufficiently-increase-receive-buffer-size/12498) when starting IPFS: `sudo sysctl -w net.core.rmem_max=2500000`
 
13. When everything is ready, start the IPFS server: 
`GOLOG_LOG_LEVEL=error,motrds=debug cmd/ipfs/ipfs daemon`.
This will start the kubo server with the default logging level set to only print errors except for the motrds plugin which will be logging in debug mode.
