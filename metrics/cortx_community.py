...
class CortxPerson:
    ...
    def __str__(self):
        return ("%s at company %s email %s type %s linkedin %s %s" % (
        self.login, self.company, self.email, self.type, self.linked, " Notes: %s" % self.note if self.note else ""))
...
def get_repos(gh=None, org_name='Seagate', prefix='cortx'):
    ...
    repos = []
    for repo in orepos:
        if (prefix and prefix not in repo.name) or repo.name.endswith('.old') or repo.name.endswith(
                '-old') or repo.private:
            continue
        else:
            repos.append(repo)
...
def add_gids(gids, successes, team_url):
    ...
    for gid, email in sorted(gids.items()):
        if email not in successes:
            ...
            print(
                "%25s [%35s] added to memberships %s: %s" % (gid, email, team_url, r.status_code == requests.codes.ok))
