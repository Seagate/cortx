...
{
	struct clovis_utility_param cat_param;
	int                         rc;
	char                       *dest_fname = NULL;

	clovis_utility_args_init(argc, argv, &cat_param,
			         &dix_conf, &clovis_conf, &cat_usage);
	...
	rc = clovis_read(&clovis_container, cat_param.cup_id, dest_fname,
		          cat_param.cup_block_size, cat_param.cup_block_count,
			  cat_param.cup_take_locks);
	if (rc < 0) {
		fprintf(stderr, "clovis_read failed! rc = %d\n", rc);
	}
