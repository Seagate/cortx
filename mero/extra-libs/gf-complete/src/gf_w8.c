...
#ifdef __KERNEL__
#	if !SSE_ASM_ENABLE
	assert(0 && "There's no emmintrin iside kernel -- configuration error");
#	endif
  kernel_fpu_begin();
#endif
...
#if SSE_ASM_ENABLE
  asm volatile("movdqu %0,%%xmm1" : : "m" (*bl));
  asm volatile("movdqu %0,%%xmm0" : : "m" (*bh));
  asm volatile("movdqu %0,%%xmm3" : : "m" (cf[0]));
#else
  ...
  if (xor) {
    while (sptr < (uint8_t *) rd.s_top) {
	    ...
	    asm volatile("movdqa %0,%%xmm4" : : "m" (*sptr));

	    //t1 = _mm_and_si128 (loset, va);
	    asm volatile("movdqa %xmm3,%xmm5"); // move loset to register xmm5
	    asm volatile("pand   %xmm4,%xmm5"); // t1 = xmm5

	    //r = _mm_shuffle_epi8 (mtl, t1);
	    asm volatile("movdqa %xmm1,%xmm6"); // move mtl to register xmm6
	    asm volatile("pshufb %xmm5,%xmm6"); // r = xmm6

	    //va = _mm_srli_epi64 (va, 4);
	    asm volatile("psrlq  $4,%xmm4");
	    ...
	    asm volatile("movdqa %xmm0,%xmm7"); // move mtl to register xmm7
	    asm volatile("pshufb %xmm5,%xmm7"); //
	    asm volatile("pxor   %xmm7,%xmm6"); // r = xmm6

	    //va = _mm_load_si128 ((__m128i *)(dptr));
	    asm volatile("movdqa %0,%%xmm4" : : "m" (*dptr));

	    //r = _mm_xor_si128 (r, va);
	    asm volatile("pxor   %xmm6,%xmm4"); //

	    //_mm_store_si128 ((__m128i *)(dptr), r);
	    asm volatile("movdqa %%xmm4,%0" : "=m" (*dptr));
#else
	    va = _mm_load_si128 ((__m128i *)(sptr));
	    t1 = _mm_and_si128 (loset, va);
	    r = _mm_shuffle_epi8 (mtl, t1);
    ...
    while (sptr < (uint8_t *) rd.s_top) {
	    ...
	    //_mm_store_si128 ((__m128i *)(dptr), r);
	    asm volatile("movdqa %%xmm6,%0" : "=m" (*dptr));
#else
	    va = _mm_load_si128 ((__m128i *)(sptr));
	    t1 = _mm_and_si128 (loset, va);
	    r = _mm_shuffle_epi8 (mtl, t1);
...
#	endif
  kernel_fpu_end();
#endif
#undef SSE_ASM_ENABLE

  gf_do_final_region_alignment(&rd);
}
#else /* in some configurations we need this for compilation */
static void gf_w8_split_multiply_region_sse(gf_t *gf, void *src, void *dest,
					    gf_val_32_t val, int bytes, int xor)
{
	assert(0 && "Your processor doesn't support SSSE3"
	       " -- configuration error");
}
#endif
