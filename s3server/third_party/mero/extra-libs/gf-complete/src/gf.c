...
/**
 * Fetch cpu flags (applicable for AMD and Intel).
 * https://en.wikipedia.org/wiki/CPUID#EAX.3D1:_Processor_Info_and_Feature_Bits
 */
static inline void cpuid_flags(unsigned int *eax, unsigned int *ebx,
...
static int is_ssse3_present(void)
{
	unsigned eax;
	unsigned ebx = 0;
	unsigned ecx = 0;
...
#if defined(X86_64_AMD64) || defined(__KERNEL__)
    sse3 = is_ssse3_present();
#endif
...
#	endif
  kernel_fpu_begin();
    ...
    while (s8 < (uint8_t *) rd.s_top) {
	    ...
	    asm volatile("movdqa %0,%%xmm0" : : "m" (*s8));

	    //md = _mm_load_si128 ((__m128i *)(d8));
	    asm volatile("movdqa %0,%%xmm1" : : "m" (*d8));

	    //md = _mm_xor_si128(md, ms);
	    asm volatile("pxor   %xmm0,%xmm1"); // md = xmm1

	    //_mm_store_si128((__m128i *)(d8), md);
	    asm volatile("movdqa %%xmm1,%0" : "=m" (*d8));
	    s8 += 16;
	    d8 += 16;
    }
  ...
  while (d8 < (uint8_t *) dest + abytes) {
	  ...
	  asm volatile("movdqu %0,%%xmm0" : : "m" (*s8));

	  //md = _mm_loadu_si128 ((__m128i *)(d8));
	  asm volatile("movdqu %0,%%xmm1" : : "m" (*d8));

	  //md = _mm_xor_si128(md, ms);
	  asm volatile("pxor   %xmm0,%xmm1"); // md = xmm1

	  //_mm_storeu_si128((__m128i *)(d8), md);
	  asm volatile("movdqu %%xmm1,%0" : "=m" (*d8));
